...
describe('Test GroupController', () => {
	it("Creates a group", async () => {
		const response = await request(app).post("/group").set('Authorization', 'Bearer creatorId')
			.send({
				"name": "group name",
				"users": [
					"user1",
	...
	it("Updates a group", async () => {
		const responseCreate = await request(app).post("/group").set('Authorization', 'Bearer creatorId')
			.send({
				"name": "group name",
				"users": ["user1", "user2"],
			});
		...
		const responseUpdate = await request(app).put(`/group/${groupId}`).set('Authorization', 'Bearer creatorId')
			.send({
				"name": "new name",
